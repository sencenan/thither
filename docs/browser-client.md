...
4. Read the register. For an `R` on initial URL-driven execution, navigate when the input was non-empty, the result holds exactly one match with a nonnegative argument balance (one selected target with a best fit, or with a single variant), **and the persisted stack did not change**; otherwise show the fallback UI. There is no special case forcing a bare URL to the fallback UI. **Empty input never auto-navigates**: an empty program searches on focus alone and matches every target, so a one-target state would otherwise redirect on a blank open and never be reachable — opening the launcher with no input always shows the page, keeping targets addable and Settings reachable. For an `E`, display its `type` and `description` without interpreting the type value; when `loaded` is `false`, add that recovery happens through the Settings reset.

The persisted stack never changes on a plain search, so the `changed` guard only bites after a mutation: `https://example.com/ home .set` sets and then shows the fallback UI rather than setting and navigating in one keystroke. Automatic navigation uses `location.replace`, so the Thither page does not remain in history as a redirect loop behind the destination; links in the fallback UI are ordinary anchors.
