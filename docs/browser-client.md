...
4. Read the register. For an `R` on initial URL-driven execution, navigate when every match shares one key (a single selected target) that has a best fit — the `argDelta`-0 row is the destination — or is a single-variant target whose one row has a nonnegative argument balance, **and `R.inputs` is non-empty** — the user typed a query ([ADR 0009](adr/0009-auto-navigate-on-a-non-empty-query.md), [ADR 0011](adr/0011-fan-out-variants-client-picks-navigable.md)). A target's sibling variants no longer block navigation; matches from a second selected target do. Otherwise show the fallback UI. There is no special case forcing a bare URL to the fallback UI, and no check on whether the run mutated state. For an `E`, display its `type` and `description` without interpreting the type value; when `loaded` is `false`, add that recovery happens through the Settings reset.
...
The fallback page has a text field at the top containing the current program input. Editing it updates the query and triggers live execution after a **60 ms keystroke debounce**, rather than waiting for Enter; each keystroke restarts the delay. Live execution uses the ordinary interpreter and persistence flow, including mutations and bounded history — it is not a separate read-only search. **A program ending in `.set` or `.rm` that ran to its search leaves the field empty**, whether it arrived by URL or was typed: the mutation is done, the list beneath already shows every target (the empty query’s result, with the change in it), and typing on from that text would only re-run the mutation. A program ending in `.@` that ran to its search clears the field the same way: the focus is set, the focus bar beneath the field now shows it, and typing on would only re-run it. A mutation that errored keeps its text so it can be corrected, and a program that continues past the mutation (`<url> home .set home`) is left as typed. The field takes focus when the page is shown, and a printable key pressed while it is not focused returns focus to it and types there: the page has no other typing target. Pressing Escape (while the Settings dialog is closed) clears the field, returns focus to it, and re-runs, resetting the query.
...
When the current focus (the register's `state`) is non-empty, a focus bar directly beneath the field names the dimensions every search is prefixed with, so the user sees what `.@` set; it is absent when focus is empty.

Below the field, display results in the order `R.matches` already has; the core emits its final display order, so the client does not sort. Each row shows the target's key, its destination, its argument balance, and `hint.score`. Indicate which characters of the key matched from `hint.positions`, and which arguments were inserted by rendering the match's destination template ([dsl.md](dsl.md) §5) with its applied arguments slot by slot — a filled `{}` shows its argument, an unfilled one stays visible as a placeholder — rather than searching the rendered destination or re-running the matcher. The applied arguments are also listed after the key, behind a leading `.` separator (the same `.` dsl.md uses between the matching portion and the arguments, shown only when arguments were applied), so the row states what was filled in even where the rendered destination is long or truncated. A row missing arguments reads visibly different from a complete one.

Every match is listed. The match count and key hints appear in a summary bar directly beneath the field, above the list, and sticky so they stay visible while a long list scrolls. The first ten rows have navigation shortcuts `Ctrl+1` through `Ctrl+9`, then `Ctrl+0` for the tenth, active whether or not the text field has focus; display the digit beside each of those rows, and nothing beside the rest. **Enter opens the first row when the run searched a query** — `R.inputs` non-empty, the same test automatic navigation applies ([ADR 0009](adr/0009-auto-navigate-on-a-non-empty-query.md)) — so `https://example.com/ home .set` followed by Enter commits and lists rather than opening the first of every target; a pending debounce is run first, so Enter acts on what was typed. A shortcut, Enter, or a click opens the row's destination as rendered, an unfilled `{}` included — only automatic navigation demands a complete destination.

While the target set is empty (the register's `state` has no targets), the page shows setup instructions in place of the result list: the default (`?q=`) shortcut URL template, one example `.set` program, and one example fuzzy (fzf) search that shows how to match a target and fill its arguments. Only the `?q=` template appears here — Help carries the fuller reference including `#q=` — to keep the empty state compact. They disappear once the target set is non-empty and are not reachable from a dedicated URL. They are not shown when `loaded` is false.

Provide a Help control beside the field, opening a modal that gives a comprehensive reference — how to search, the language's basic rules (the four operations, arguments and the separator, search operators, escaping), and setup instructions with this page's shortcut templates. Unlike the empty-target-set setup instructions, Help is always reachable.
