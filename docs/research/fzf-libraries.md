...
> **Outcome:** Thither adopted the npm `fzf` port as an ordinary dependency, with per-dimension AND matching, summed scores, target-set order for ties, and diacritic normalization disabled. See [dsl.md](../dsl.md) and [browser-client.md](../browser-client.md). This note is the research snapshot that led there; its open questions are now closed.

## Recommendation

**Go is the strongest choice for fidelity to fzf: use fzf's own matcher. TypeScript/JavaScript is the practical choice for a browser-based Thither: use the `fzf` npm package, accepting a pinned port rather than claiming compatibility with current upstream fzf.** Rust's `nucleo-matcher` is worth considering for a native application, but explicitly differs from fzf. These are architectural recommendations based on the primary-source findings below, not benchmark results.

No stack decision is made by this note. Thither's browser and deployment requirements remain open.
...
The higher-level query machinery is separate: upstream `pattern.go` parses extended terms and sums their match scores. Calling `FuzzyMatchV2` alone does not reproduce query parsing or list ranking. For Thither, bypassing the extended query language is desirable because the DSL prohibits its operators. [2, 9]

Recommendation: place the upstream package behind a small adapter, pin its revision, and own Thither's multi-token and tie-breaking policies. This is a source-level dependency on the fzf project; this investigation did not establish an independently guaranteed stable matcher API.
...
Useful options include `fuzzy: "v2"`, `casing: "case-insensitive"`, normalization, selectors, and tie-breakers. The documented defaults include smart case, normalization enabled, basic matching, and an unlimited result count. Explicit configuration is preferable for Thither. [5]

**Important integration trap:** `basicMatch` sends one query pattern to the algorithm. `extendedMatch` parses multiple terms but also imports fzf's special operators. Neither should silently decide Thither's unresolved multi-token policy. If Thither chooses independent AND-matched dimensions with summed scores, implement that composition using literal fuzzy patterns rather than passing raw user input to the extended parser. [5, 9]
...
3. **Unicode and normalization:** Thither's dimension lowercasing is not automatically the same as a matcher's optional diacritic normalization.
...
9. [Thither's DSL specification](../dsl.md), especially sections 5, 7, and 10.

Method: primary documentation and source inspection; no library installation in Thither, runtime benchmarks, or differential compatibility tests. No background-agent facility was available, so research was performed directly.
