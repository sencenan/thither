...
**Superseded by [ADR 0006](0006-clients-resume-by-executing-on-the-persisted-stack.md).** `execute` now takes the stack to evaluate on, so the program carries only input tokens.
