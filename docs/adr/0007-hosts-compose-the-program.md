...
- Reset and restore are programs too — `[...values, '.$', '.out', '.save']` — so supplied values are validated by `pushToken` and history follows the same difference rule. The reset UI dry-runs `[...values, '.$', '.out']` first and refuses to save when the register holds an `E`.
