...
- `.out` moves the top of the stack into the **output register** when it is an `R` or `E`, and does nothing otherwise. It then records the `S` left on top, when there is one, as the register's `state` — the world as the run left it, after any mutation the user's tokens made, which is what the fallback UI reads (`.load` sees only the world before them). It extracts the run's output for the host's next step; it is not a general pop.
- `.save` persists the stack as it stands, applying the bounded-history difference rule — but only when the register holds an `R`. The `R` proves `.$` ran and consumed the literals, so the stored stack is always one `.load` can read; a run that ended in an `E` persists nothing. (It originally wrote whenever the stack was non-empty, and a parse-phase `E` — which seals the stack over the literals it interrupted — let `.out` pop the `E` and `.save` write the orphaned `L`, a record `.load` then rejected. Live execution made that reachable by pausing while typing an operation name.) A save whose write fails pushes `E(unknown_error)` and writes it into the register as the run's terminal. (It originally also recorded `changed`, a structural-difference flag the client's navigation rule consulted; [ADR 0009](0009-auto-navigate-on-a-non-empty-query.md) dropped it, along with the register's `saved` field.)
